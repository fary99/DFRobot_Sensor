 ...
 * @author [Fary](Fary.young@dfrobot.com)
 * @version  V1.0
 * @date  2020-11-20
 * @https://github.com/fary99/DFRobot_Sensor
 */
#ifndef _DFROBOT_SENSOR_H_
#define _DFROBOT_SENSOR_H_
...
#define DFRobot_Sensor_IIC_ADDR  0x66        //芯片的IIC地址，无地址变化功能
#define DFRobot_Sensor_ID        0xDF        //芯片的IIC地址，无地址变化功能
...
public:
  ...
  */
  typedef enum{
    ...
    eNomalPercision = 1<<2,  /**< 正常精度，精度大概在xxx，在正常精度模式下，可以进入低功耗 */
    eHighPercision  = 2<<2,  /**< 高精度，精度大概在xxx，在高精度模式下，采集速率会降低，采集周期100ms，不能进入低功耗 */
    eUltraPercision = 3<<2,  /**< 超高精度，精度大概在xxx，在超高精度模式下，采集速率会极低，采集周期1000ms，不能进入低功耗 */
  }ePercisionMode_t;
...
public:
  ...
  /**
   * @brief 初始化函数
   * @return 返回0表示初始化成功，返回其他值便是初始化失败，返回错误码
   */
...
public:
  ...
  DFRobot_Sensor_IIC(TwoWire *pWire = &Wire,
                     uint8_t mode = eNomalPercision + eNormalSpeed + eNormalPower);
